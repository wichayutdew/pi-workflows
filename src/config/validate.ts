...
function parseBashRule(value: unknown, path: string, errors: string[]): BashRule | undefined {
  if (!isObject(value)) {
    errors.push(`${path}: expected an object`);
    return undefined;
  }
  rejectUnknownKeys(value, ["executable", "argsPrefix"], path, errors);
...
function parseBashPermission(
  ...
  const mode = readString(value.mode, `${path}.mode`, errors) as BashMode | undefined;
  const validMode =
    mode === "deny" ||
    mode === "read-only" ||
    mode === "allow-list" ||
  ...
  if (value.allow !== undefined) {
    if (!Array.isArray(value.allow)) {
      ...
      value.allow.forEach((rule, index) => {
        const parsed = parseBashRule(rule, `${path}.allow[${index}]`, errors);
        if (parsed) allow.push(parsed);
      });
    }
  }
  ...
  if (normalizedMode === "allow-list" && allow.length === 0) {
    errors.push(`${path}.allow: at least one rule is required for allow-list mode`);
  }
...
function parseStepSubagent(
  ...
  if (value === undefined) return { ...DEFAULT_STEP_SUBAGENT };
  if (!isObject(value)) {
    errors.push(`${path}: expected an object`);
    return { ...DEFAULT_STEP_SUBAGENT };
  }
  rejectUnknownKeys(
    value,
    ["agent", "context", "model", "timeoutMs", "turnBudget", "toolBudget", "artifacts"],
    path,
    errors,
  );
...
function parsePrompt(value: unknown, path: string, errors: string[]): PromptSpec | undefined {
  if (typeof value === "string") {
...
function parseGate(value: unknown, path: string, errors: string[]): PlannotatorGate | undefined {
  if (value === undefined) return undefined;
  if (!isObject(value)) {
    errors.push(`${path}: expected an object`);
    return undefined;
  ...
  rejectUnknownKeys(
    value,
    ["provider", "submitOutcome", "approvedOutcome", "rejectedOutcome", "timeoutMs"],
    path,
    errors,
  );
  ...
  const timeoutMs = readInteger(value.timeoutMs, 5_000, `${path}.timeoutMs`, errors, {
    min: 1_000,
    max: 30_000,
  });

  if (provider !== "plannotator") {
    errors.push(`${path}.provider: only "plannotator" is supported`);
  }
  if (!submitOutcome || !approvedOutcome || !rejectedOutcome || provider !== "plannotator") {
    return undefined;
  }
...
function parseStep(value: unknown, stepId: string, path: string, errors: string[]): WorkflowStep | undefined {
  if (!isObject(value)) {
    errors.push(`${path}: expected an object`);
    return undefined;
  }
  rejectUnknownKeys(
    value,
    ["title", "prompt", "subagent", "permissions", "requires", "transitions", "gate"],
    path,
    errors,
  );
  ...
  if (gate) {
    ...
    if (!permissions.extensions.includes("plannotator")) {
      errors.push(`${path}.permissions.extensions: a Plannotator gate must allow "plannotator"`);
    }
    if (!requires.extensions.includes("plannotator")) {
      errors.push(`${path}.requires.extensions: a Plannotator gate must require "plannotator"`);
    }
  }
...
export function validateWorkflow(value: unknown): ValidationResult<WorkflowDefinition> {
  const errors: string[] = [];
  if (!isObject(value)) {
  ...
  const id = readString(value.id, "workflow.id", errors, { pattern: IDENTIFIER_PATTERN });
  ...
  const start = readString(value.start, "workflow.start", errors, { pattern: IDENTIFIER_PATTERN });
...
function parsePermissionCeiling(
  ...
  const subagent = parseSubagentPermissionCeiling(
    value.subagent,
    `${path}.subagent`,
    errors,
  );
  if (!subagent) return undefined;
  return { ...permissions, subagent };
}
...
export function validateSettings(value: unknown): ValidationResult<WorkflowSettings> {
  const errors: string[] = [];
  if (!isObject(value)) {
