...
function normalizeStartResponse(value: unknown): PlannotatorStartResponse {
  if (value === null || typeof value !== "object") {
    return { status: "error", error: "Plannotator returned an invalid response" };
  }
  ...
  if (response.status === "error") {
    return { status: "error", error: errorText(response, "Plannotator failed") };
  }
  const result =
  ...
  return { status: "error", error: "Plannotator returned an invalid start result" };
}
...
function normalizeStatusResponse(
  ...
  if (response.status !== "handled" || !result) {
    return { status: "error", error: "Plannotator returned an invalid status result" };
  }
...
export function parsePlannotatorResult(value: unknown): PlannotatorReviewResult | undefined {
