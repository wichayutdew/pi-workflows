...
import { createEmptyCatalog } from './catalog.ts';
import {
...
import { createStatusActions } from './status-actions.ts';
import { createStartActions } from './start-actions.ts';
import { createPauseActions } from './pause-actions.ts';
import { createResumeAction } from './resume-action.ts';
import { createLifecycleActions } from './lifecycle-actions.ts';
import { createStepExecutionActions } from './step-execution-actions.ts';
...
/**
 * Creates and registers a functional workflow harness against the Pi extension API.
 *
 * @param pi - Pi extension API used for command, tool, and session registration.
 * @param statusShortcut - Keyboard shortcut that toggles the status overlay.
 * @param dependencyOverrides - Optional mock dependencies for testing.
 * @returns A workflow command controller.
 */
