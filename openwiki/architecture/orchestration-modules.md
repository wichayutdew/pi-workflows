...
| ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| `src/domain/`                   | Shared configuration, run-state, step-result, policy, agent, Plannotator, profile, status, and harness types.                     |
...
| `src/function/preflight/`       | Uses tool and skill inventories to report missing tools, extensions, skills, MCP support, Plannotator, or pi-agents before execution. |
...
| `src/infrastructure/fs/catalog.ts`           | Composes the injected settings/workflow loaders, duplicate checks, command ownership checks, and final catalog diagnostics.                 |
...
| `src/function/config/validation/settings.ts`    | Validates user-owned settings, currently schema version and status shortcut, and applies defaults.                                      |
...
| `src/infrastructure/harness/dependencies.ts`                   | Defines and supplies the effect ports for time, IDs, aborts, files, catalog loading, reviews, diagnostics, agents, runtimes, queues, timers, and status UI.                                  |
| `src/infrastructure/harness/gate-submission-action.ts`         | Converts a gated `ready` result into pending review, or records recoverable artifact-contract feedback as a same-step handoff before Plannotator submission.                                      |
...
| `src/infrastructure/harness/pause-actions.ts`                  | Implements manual pause and abort, including active main-step, Plannotator-review, and delegation cleanup.                                                                                      |
...
| `src/infrastructure/harness/step-execution-actions.ts`         | Selects main versus delegated execution, activates main policy, contains synchronous agent startup exceptions, and queues results.                                                           |
